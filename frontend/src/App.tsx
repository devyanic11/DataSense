...
function App() {
  ...
  const colCount = insightData?.column_meta
    ? Object.keys(insightData.column_meta).length
    : 0;
  ...
  return (
    ...
    >
      {/* ─── TOPBAR ─────────────────────────────────────────── */}
      <header
        className="h-12 flex items-center px-4 gap-4 shrink-0 z-40"
        style={{
      ...
      >
        ...
        <div className="flex-1 flex justify-center">
          {insightData ? (
            ...
            >
              {insightData.filename} · {rowCount.toLocaleString()} rows ·{" "}
              {colCount} cols
            </span>
          ) : (
        ...
        <div className="flex items-center gap-2">
          {insightData && (
            <button
              ...
              style={{
                color: exporting ? "var(--accent)" : "var(--text-muted)",
              }}
              disabled={exporting}
              onMouseEnter={(e) =>
                !exporting &&
              ...
              onClick={async () => {
                ...
                try {
                  await generateReport(insightData, (status) =>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <main
          ...
          style={{ marginLeft: 52, height: "calc(100vh - 48px)" }}
        >
          {!insightData ? (
            <FileUpload onSuccess={handleUploadSuccess} />
          ) : activeView === "dashboard" ? (
            <div
              style={{ maxWidth: 1400, margin: "0 auto", padding: "24px 32px" }}
            >
              <Dashboard
                data={insightData}
                externalChartRequest={chartRequest}
              />
            </div>
          ) : activeView === "table" ? (
            <div
            ...
            >
              ...
              >
                Showing {Math.min(50, rowCount)} of {rowCount.toLocaleString()}{" "}
                rows
              </div>
              <div
              ...
              >
                ...
                >
                  <thead>
                    <tr>
                      {Object.keys(insightData.original_data[0] || {}).map(
                        (col: string) => (
                          <th
                            key={col}
            ...
            >
              ...
              >
                <thead>
                  <tr>
                    {["Column", "Type", "Unique", "Min", "Max", "Nulls"].map(
                      (h) => (
                        <th
                          key={h}
                          className="text-left"
                ...
                <tbody>
                  {Object.entries(insightData.column_meta).map(
                    ([col, meta]: [string, any]) => (
                      <tr
                        key={col}
                        style={{ borderBottom: "1px solid var(--border-dim)" }}
        ...
        <div
          className={
            chatPinned
              ? "chat-panel-pinned flex flex-col w-[380px] min-w-[380px] border-l border-[var(--border)] bg-[var(--bg-surface)]"
              : `chat-panel ${chatOpen ? "open" : ""}`
          }
        ...
        >
          {insightData && (
            <Chat
              ...
              onChartRequested={(type, data) =>
                handleChartRequested(type, data)
              }
