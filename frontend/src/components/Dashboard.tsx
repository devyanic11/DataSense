...
function generateKPIs(
  ...
  const kpis: KPIItem[] = [];
  const entries = Object.entries(columnMeta).filter(
    ([k]) => !k.startsWith("_"),
  );

  const isIdentifier = (col: string, meta: any): boolean => {
    const name = col.toLowerCase();
    if (/^(year|yr|id|index|row|serial|code|zip|pin|phone)/.test(name))
      return true;
  ...
  for (const [col, meta] of entries) {
    ...
    if (meta.type === "numeric") {
      if (isIdentifier(col, meta)) {
        ...
        if (meta.nunique != null)
          stats.push({ label: "Unique", value: fmt(meta.nunique) });
        kpis.push({ colName: col, kind: "identifier", stats });
      } else {
        if (meta.mean != null)
          stats.push({ label: "Mean", value: fmt(meta.mean) });
        if (meta.median != null)
          stats.push({ label: "Median", value: fmt(meta.median) });
        if (meta.std != null && meta.std > 0)
          stats.push({ label: "Std Dev", value: fmt(meta.std) });
        if (meta.mode != null)
          stats.push({ label: "Mode", value: fmt(meta.mode) });
        if (meta.min != null && meta.max != null)
          stats.push({
            label: "Range",
            value: `${fmt(meta.min)} – ${fmt(meta.max)}`,
          });
      ...
      // Categorical
      if (meta.nunique != null)
        stats.push({ label: "Unique", value: String(meta.nunique) });
      if (meta.top_values) {
      ...
      if (meta.null_count > 0)
        stats.push({ label: "Missing", value: String(meta.null_count) });
...
export default function Dashboard({
  ...
  const initialTabs: ChartConfig[] =
    data.chart_configs?.length > 0
      ? data.chart_configs
      : (data.insights.suggested_charts || []).map((type) => ({
          type,
          title: type,
        }));
  ...
  const dateColumn = useMemo(() => {
    ...
    for (const [colName, colInfo] of Object.entries(data.column_meta)) {
      if (
        colInfo?.type === "date" ||
        colInfo?.type === "datetime" ||
        colInfo?.type === "timestamp"
      ) {
        return colName;
      }
    }
  ...
  const applyDateFilter = useCallback(() => {
    if (!filterStartDate || !filterEndDate || !dateColumn) {
      console.log("[Filter] ❌ No dates or date column selected", {
        filterStartDate,
        filterEndDate,
        dateColumn,
      });
    ...
    console.log(`[Filter] 🔍 Filtering on column: "${dateColumn}"`, {
    ...
    console.log(
      `[Filter] ✅ Date filter applied: ${filterStartDate} to ${filterEndDate} | Matches: ${filtered.length} of ${data.original_data?.length || 0} rows`,
      filtered.slice(0, 3),
    );
  ...
  const downloadChart = () => {
    ...
    if (gd && (window as any).Plotly) {
      (window as any).Plotly.downloadImage(gd, {
        ...
        width: 1200,
        height: 700,
        filename: `${activeChart.title || "chart"}_datasense`,
      });
    }
  };
  ...
  const autoConfigForType = (chartTypeName: string): ChartConfig => {
    ...
    if (type.includes("bar"))
      return {
        type: chartTypeName,
        title: chartTypeName,
        x_key: categoricalCols[0] || keys[0],
        y_keys: numericCols.slice(0, 3),
      };
    if (
      type.includes("line") ||
      type.includes("area") ||
    ...
    if (type.includes("pie"))
      return {
        type: chartTypeName,
        title: chartTypeName,
        label_key: categoricalCols[0] || keys[0],
        value_key: numericCols[0] || keys[1],
      };
    if (type.includes("scatter") || type.includes("plot"))
      return {
        type: chartTypeName,
        title: chartTypeName,
        x_key: numericCols[0] || keys[0],
        y_key: numericCols[1] || numericCols[0] || keys[1],
        tooltip_key: categoricalCols[0],
      };
  ...
  useEffect(() => {
    ...
    setChartTabs((prevTabs) => {
      const existsIdx = prevTabs.findIndex(
        (t) =>
      ...
      if (existsIdx >= 0) {
        setTimeout(() => {
          ...
          chartAreaRef.current?.scrollIntoView({
            behavior: "smooth",
            block: "start",
          });
        }, 50);
  ...
  const rawRows = useMemo(
    () => prepareRows(filteredData || []),
    [filteredData],
  );
  const kpis = useMemo(
    () => generateKPIs(data.column_meta || {}, data.original_data || []),
    [data.column_meta, data.original_data],
  );

  useEffect(() => {
    const isGraph =
      chartType.includes("graph") ||
      chartType.includes("network") ||
      chartType.includes("knowledge");
  ...
  const getIcon = (type: string, size = 14) => {
    ...
    if (t.includes("pie") || t.includes("sunburst") || t.includes("treemap"))
    ...
    if (t.includes("line") || t.includes("trend") || t.includes("area"))
    ...
    if (t.includes("graph") || t.includes("network") || t.includes("knowledge"))
  ...
  const autoGenerateMapping = useCallback(
    (chartType: string) => {
      const keys = rawRows.length > 0 ? Object.keys(rawRows[0]) : [];
      const numericCols = keys.filter(
        (k) => typeof rawRows[0]?.[k] === "number",
      );
      const categoricalCols = keys.filter(
        (k) => typeof rawRows[0]?.[k] === "string",
      );

      const type = chartType.toLowerCase();
      console.log(
        `[autoGenerateMapping] Chart: ${chartType} | Numeric: ${numericCols.length} | Categorical: ${categoricalCols.length}`,
      ...
      if (type.includes("scatter") || type.includes("plot")) {
        return {
          x_key: numericCols[0] || keys[0],
          y_keys: [numericCols[1] || numericCols[0] || keys[1]],
          tooltip_key: categoricalCols[0],
        };
      }
  ...
  const renderChart = () => {
    ...
    if (cfg.isBlank && !cfg.plotly_json) {
      return (
        ...
        >
          ...
          <p style={{ fontSize: 13 }}>
            Select a chart type below to visualize your data
          </p>
        </div>
      );
    }
    ...
    console.log(
      `[renderChart] isDataFiltered=${isDataFiltered}, filtered=${filteredData.length}, original=${data.original_data?.length}`,
    );
        ...
        if (parsed.error) {
          return (
            ...
            >
              <Info
                size={40}
                style={{ opacity: 0.4, color: "var(--danger)" }}
              />
              <p
                style={{
                  fontSize: 13,
    ...
    let mapping: any = {
      ...
      y_key: cfg.y_key,
      nbins: (cfg as any).nbins,
      columns: (cfg as any).columns,
      tooltip_key: cfg.tooltip_key,
    };

    // If critical mappings are missing, auto-generate them
    if (
      (!mapping.x_key && !mapping.label_key && !mapping.columns) ||
      (!mapping.y_keys &&
        !mapping.y_key &&
        !mapping.value_key &&
        type !== "heatmap" &&
      ...
      console.log(
        `[renderChart] Missing mappings, auto-generating for ${cfg.type}`,
      );
      mapping = autoGenerateMapping(cfg.type);
    }

    if (type.includes("bar")) {
      const xKey = mapping.x_key;
      const yKeys = mapping.y_keys || [];
    ...
    if (
      ...
      const xKey = mapping.x_key;
      const yKeys: string[] = mapping.y_keys || [];
      if (!xKey || yKeys.length === 0) return <NoColumnWarning />;
      return (
        <ResponsiveContainer width="100%" height={360}>
          <AreaChart
    ...
    if (type.includes("pie")) {
      const labelKey = mapping.label_key;
      const valueKey = mapping.value_key;
    ...
    if (type.includes("scatter") || type.includes("plot")) {
      const xKey = mapping.x_key;
      const yKey = mapping.y_key || mapping.y_keys?.[0];
      ...
      return (
        <ResponsiveContainer width="100%" height={360}>
          <ScatterChart margin={{ top: 16, right: 24, bottom: 40, left: 0 }}>
            ...
            <XAxis
              type={
                typeof rawRows[0]?.[xKey] === "number" ? "number" : "category"
              }
              dataKey={xKey}
              name={xKey}
              {...axisStyle}
            ...
            {mapping.tooltip_key && (
              <ZAxis dataKey={mapping.tooltip_key} range={[40, 280]} />
            )}
    ...
    if (type.includes("map") || type.includes("geo")) {
      return (
        <div
          ...
          style={{ color: "var(--text-muted)" }}
        >
          <MapIcon size={56} style={{ opacity: 0.3 }} />
          <p style={{ fontSize: 14 }}>Geographic Map</p>
          <p style={{ fontSize: 12 }}>Requires GeoJSON location data</p>
        </div>
      );
    }
      ...
      if (graphLoading)
        return (
          ...
          >
            ...
            />
            <p style={{ fontSize: 13 }}>
              AI is generating the Knowledge Graph…
            </p>
          </div>
        );
      if (graphNodes.length === 0)
        return (
          <div
            ...
            style={{ color: "var(--text-muted)" }}
          >
            <Network size={56} style={{ opacity: 0.3 }} />
            <p style={{ fontSize: 13 }}>No graph data could be extracted.</p>
          </div>
        );
      return (
        <InteractiveKnowledgeGraph nodes={graphNodes} edges={graphEdges} />
      );
    }
          ...
          <div className="flex flex-wrap items-end gap-3">
            ...
            <button
              ...
              style={{
                ...
                fontWeight: 600,
                cursor:
                  filterStartDate && filterEndDate ? "pointer" : "not-allowed",
            ...
            {filteredData.length !== (data.original_data?.length || 0) && (
              ...
              >
                ...
                {data.original_data?.length || 0} rows
              </div>
            )}
          </div>
        </div>
          ...
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-1 overflow-x-auto scrollbar-none">
              {chartTabs.map((cfg, idx) => {
                ...
                const isNew =
                  idx >= (data.chart_configs?.length || initialTabs.length);
                return (
                  <button
                    key={`${cfg.type}-${idx}`}
                    onClick={() => {
          ...
          >
            ...
            >
              <div>
                ...
                >
                  {selectedConfig?.title || selectedConfig?.type}
                </div>
              ...
              <div className="flex items-center gap-1">
                <button
                  ...
                  style={{
                    color: isEditing
                      ? "var(--accent-text)"
                      : "var(--text-muted)",
                    background: isEditing ? "var(--accent-dim)" : "transparent",
                  }}
                >
                  <Pencil size={14} />
                </button>
                <button
                  ...
                  disabled={!selectedConfig?.plotly_json}
                ...
                >
                  ...
                  !chartType.includes("knowledge") ? (
                    ...
                    >
                      ...
                      />
                      <p style={{ fontSize: 13 }}>
                        No tabular data for this file type.
                      </p>
                    </div>
                  ) : (
                    renderChart()
...
function NoColumnWarning() {
  return (
    ...
    >
      ...
      <p style={{ fontSize: 13 }}>
        AI chart config is missing column mappings for this chart type.
      </p>
      <p style={{ fontSize: 12 }}>
        This may happen if the file lacks matching numeric or categorical
        columns.
      </p>
    </div>
  );
